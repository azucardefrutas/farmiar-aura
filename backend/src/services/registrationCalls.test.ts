...
it('exposes public call metadata and only the requesting voter registration flag', async () => {
  ...
  const a = result([{ id: 'open', nombre: 'Abierta', status: 'registration', format: 'single_elimination', is_current: false, match_duration_seconds: 90, aura_per_vote: 100 }])
  const b = result([{ id: 'closed', nombre: 'Cerrada', status: 'finished' }])
  const registrations = result([{ tournament_id: 'open', status: 'approved' }])
  const supabase = { from: vi.fn().mockReturnValueOnce(a).mockReturnValueOnce(b).mockReturnValueOnce(registrations) }
